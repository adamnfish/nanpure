module GridTests exposing (distinct, suite)

import Expect
import Fuzz exposing (Fuzzer)
import Grid exposing (CellValue(..), Cells, Grid, Number(..), cellBySquareAndCellIndices, cellInSquare, clearCell, emptyGrid, getCell, getCellSquare, getSquare, isFilled, puzzle, setCell, valid)
                ...
                , describe "puzzle"
                    ...
                    , fuzz cellsFuzzer "fixes multiple cells" <|
                        \cells ->
                            let
                                cellsWithValues =
                                    List.map (\cell -> ( cell, One )) cells

                                gridResult =
                                    puzzle cellsWithValues
                            in
                            case gridResult of
                                Err _ ->
            ...
            [ describe "valid"
                [ test "returns true for an empty row" <|
                    \_ ->
                        let
                            cells =
                                makeCells Empty Empty Empty Empty Empty Empty Empty Empty Empty
                        in
                ...
                , test "returns true for a partial row of valid input" <|
                    \_ ->
                        let
                            cells =
                                makeCells (Input One) Empty Empty (Input Two) Empty Empty Empty Empty Empty
                        in
                        Expect.true "was not valid" (valid cells)
                ...
                , test "returns false for a row with invalid incomplete input" <|
                    \_ ->
                        let
                            cells =
                                makeCells (Input One) (Input One) Empty Empty Empty Empty Empty Empty Empty
                        in
                        Expect.false "expected invalid, retured true" (valid cells)
                , test "returns false for a row with input that clashes with fixed values" <|
                    \_ ->
                        let
                            cells =
                                makeCells (Input One) (Fixed One) Empty Empty Empty Empty Empty Empty Empty
                        in
                        Expect.false "expected invalid, retured true" (valid cells)
                ]
            ]
